...
@section('content')
    ...
    @if (! $company->hasAndroidEnterprise())
        <div class="bg-white rounded-xl shadow p-6 text-center mb-6">
            <h2 class="font-medium text-gray-800 mb-2">Connect Android Enterprise</h2>
            <p class="text-sm text-gray-500 mb-4">
                Before you can add Android devices, you need to connect
                your company's Android Enterprise account. It's free and
                requires a company Google account.
            </p>
    ...
    @if ($company->hasAndroidEnterprise() || $company->hasApnsConfigured())
        ...
        @if ($devices->isEmpty())
            <div class="bg-white rounded-xl shadow p-8 text-center text-gray-500 text-sm">
                No devices enrolled yet. Generate a QR code and scan it
                with a new (or factory-reset) Android device during initial
                setup.
            </div>
        @else
